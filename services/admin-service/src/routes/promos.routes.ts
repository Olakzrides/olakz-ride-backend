...
// ── Lifecycle actions ─────────────────────────────────────────────────────────
// activate    → manually go live immediately (scheduled → active now)
// pause       → temporarily stop an active promo  (resumable)
// resume      → restart a paused promo
// reactivate  → restore a deactivated promo back to scheduled
...
router.patch('/:promoId/reactivate',  auditMiddleware('promo_reactivate'),    ctrl.reactivate);
