...
function extractAxiosError(err: any): string {
  if (axios.isAxiosError(err)) {
    if (err.code === 'ECONNREFUSED')
      return `Payment service is not reachable (${PAYMENT_URL()})`;
    if (err.code === 'ETIMEDOUT' || err.code === 'ECONNABORTED')
      return 'Payment service timed out';
...
export class WalletService {
  ...
  static async getBalances(userId: string): Promise<{
    ...
    try {
      const res = await axios.get(
        `${PAYMENT_URL()}/api/internal/payment/wallet/balance`,
        {
   ...
   * Returns transactionId, newBalance, and bucket portions for refund routing.
   */
  static async deduct(params: {
    ...
    try {
      const res = await axios.post(
        ...
        {
          headers: { 'x-internal-api-key': INTERNAL_KEY(), 'x-user-id': params.userId },
          timeout: 8000,
        }
      );
   ...
   * Used for vendor/rider payouts and refunds.
   */
  ...
  /**
   * Refund back to the correct wallet bucket.
   * Passes cash → cash bucket and promo → promo bucket using
   * the portions stored on the original order.
   */
