...
export type ServiceType       = 'food' | 'marketplace';
...
export class VendorPromoService {
  ...
  static async getAll(restaurantId: string, filters: {
    status?: string;
    page?:   number;
    limit?:  number;
  } = {}) {
  ...
  static async create(input: CreateVendorPromoInput) {
    const {
      vendorId, restaurantId, code,
      discountPercent, maxDiscountAmount, minOrderAmount,
      totalUsesLimit, perUserLimit = null, startsAt, endsAt,
    } = input;
    ...
    // Code uniqueness within this restaurant (non-ended promos only)
    ...
    if (existing) throw new Error(`Promo code "${upperCode}" already exists for this restaurant.`);

    // Determine initial status — if starts_at is now or in the past, start active immediately
    ...
    logger.info('Vendor created food promo', { vendorId, promoId: data.id, code: upperCode, initialStatus });
    return this.format(data);
  }
  ...
  static async pause(promoId: string, vendorId: string) {
    const { data: existing, error } = await supabase
      .from('vendor_promos').select('id, status')
    ...
    logger.info('Vendor paused promo', { promoId, vendorId });
    return this.format(data);
  }
  ...
  static async resume(promoId: string, vendorId: string) {
    const { data: existing, error } = await supabase
      .from('vendor_promos').select('id, status, ends_at')
      .eq('id', promoId).eq('vendor_id', vendorId).single();
    ...
    logger.info('Vendor resumed promo', { promoId, vendorId });
    return this.format(data);
  }
  ...
  static async end(promoId: string, vendorId: string) {
    ...
    logger.warn('Vendor ended promo', { promoId, vendorId });
    return this.format(data);
  }
  ...
  static async delete(promoId: string, vendorId: string) {
    const { data: existing, error } = await supabase
      .from('vendor_promos').select('id, status, uses_count')
      .eq('id', promoId).eq('vendor_id', vendorId).single();
  ...
  static async validateCode(params: {
    code:         string;
    storeRef:     string;   // restaurant_id
    serviceType?: ServiceType;  // optional — food service always uses 'food'
    customerId:   string;
    subtotal:     number;
  }): Promise<ValidatePromoResult> {
    const { code, storeRef, customerId, subtotal } = params;
    const upperCode = code.trim().toUpperCase();

    const { data: promo, error } = await supabase
      .from('vendor_promos')
      .select('*')
      .eq('restaurant_id', storeRef)
      .eq('code', upperCode)
      .maybeSingle();
      ...
      if ((userUses ?? 0) >= perUserLimit) {
        return {
          ...
          message: perUserLimit === 1
            ...
            : `You have reached the maximum of ${perUserLimit} uses for this promo.`,
        };
      }
    }
  ...
  // ── Record use after order placed ──────────────────────────────────────────

  static async recordUse(params: {
    promoId:        string;
    userId:         string;
    orderId:        string;
    serviceType?:   ServiceType;  // optional — defaults to 'food' in food service
    discountAmount: number;
  }): Promise<void> {
    try {
      await supabase.from('vendor_promo_uses').insert({
        ...
        service_type:    params.serviceType ?? 'food',
        discount_amount: params.discountAmount,
        used_at:         new Date().toISOString(),
      });
      await supabase.rpc('increment_vendor_promo_uses', { promo_id_param: params.promoId });
    } catch (err: any) {
      logger.error('Failed to record promo use (non-fatal)', { error: err.message, ...params });
    }
  }

  // ── Usage history (vendor view) ────────────────────────────────────────────

  static async getUses(promoId: string, vendorId: string, filters: {
    page?:  number;
    limit?: number;
  } = {}) {
    const { page = 1, limit = 20 } = filters;
    ...
    const { data: promo } = await supabase
      .from('vendor_promos').select('id').eq('id', promoId).eq('vendor_id', vendorId).single();
    if (!promo) throw new Error('Promo not found');

    const { data: uses, count, error } = await supabase
      .from('vendor_promo_uses')
      .select('id, user_id, order_id, discount_amount, used_at', { count: 'exact' })
      .eq('promo_id', promoId)
    ...
    return {
      uses: (uses ?? []).map((u, idx) => {
        ...
        return {
          ...
          id:             u.id,
          user:           user
            ? { id: u.user_id, name: `${user.first_name ?? ''} ${user.last_name ?? ''}`.trim() }
            : { id: u.user_id, name: 'Unknown' },
          orderId:        u.order_id,
          discountAmount: parseFloat(u.discount_amount),
          usedAt:         u.used_at,
        };
      }),
      pagination: { page, limit, total: count ?? 0, pages: Math.ceil((count ?? 0) / limit) },
  ...
  // ── Scheduler — called by the promo status sync job ───────────────────────

  /**
   * Sync promo statuses based on current time and budget.
   * Called on service startup and every minute.
  ...
  static async syncStatuses(): Promise<void> {
    ...
    // scheduled → active (starts_at has arrived)
    ...
    if (activateError) {
      logger.error('Promo sync: failed to activate scheduled promos', { error: activateError.message });
    }

    // active → ended (ends_at has passed)
    ...
    if (expireError) {
      logger.error('Promo sync: failed to expire ended promos', { error: expireError.message });
    }

    // active → ended (budget exhausted: uses_count >= total_uses_limit)
    // Supabase doesn't support column comparison in filters directly,
    // so we fetch candidates and update individually
    const { data: exhausted } = await supabase
      .from('vendor_promos')
      .select('id, uses_count, total_uses_limit')
      .eq('status', 'active')
      .eq('service_type', 'food')
      .not('total_uses_limit', 'is', null);

    const exhaustedIds = (exhausted ?? [])
      .filter(p => (p.uses_count ?? 0) >= (p.total_uses_limit ?? Infinity))
      .map(p => p.id);
