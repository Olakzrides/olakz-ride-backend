...
export class VendorService {
  /**
   * Register a new auto mech vendor (linked to the authenticated user).
   */
  async createVendor(userId: string, dto: CreateVendorDto): Promise<AutoMechVendor> {
    ...
    if (error) {
      logger.error('Create auto mech vendor error:', error);
  ...
  /**
   * Fetch vendor profile by ID (includes services for Profile screen).
   */
  async getVendorById(vendorId: string): Promise<AutoMechVendor & { services: any[]; operatingHoursDisplay: string }> {
    const { data, error } = await supabase
      .from('auto_mech_vendors')
   ...
   */
  async updateVendor(vendorId: string, userId: string, dto: UpdateVendorDto): Promise<AutoMechVendor> {
    const { data: existing } = await supabase
      .from('auto_mech_vendors')
      .select('id, user_id')
      .eq('id', vendorId)
   ...
   */
  async searchVendors(query: SearchVendorsQuery): Promise<PaginatedResult<AutoMechVendor>> {
    const { latitude, longitude, radiusKm = 10, category, query: textQuery, page = 1, limit = 20 } = query;

    let dbQuery = supabase
      .from('auto_mech_vendors')
      .select('*, auto_mech_services!inner(id, category, is_active)', { count: 'exact' })
      .eq('status', 'approved')
      .eq('auto_mech_services.is_active', true);

    if (category)   dbQuery = dbQuery.eq('auto_mech_services.category', category);
   ...
   */
  async getTopRatedVendors(latitude: number, longitude: number, limit = 10): Promise<AutoMechVendor[]> {
    const { data, error } = await supabase
      .from('auto_mech_vendors')
      .select('*')
      .eq('status', 'approved')
  ...
  private mapRow(row: any): AutoMechVendor {
  ...
  async getStoreDetails(userId: string) {
    ...
    return {
      ...
      phone:                          data.phone,
      business_type:                  (platformVendor as any)?.business_type ?? 'auto_mech',
