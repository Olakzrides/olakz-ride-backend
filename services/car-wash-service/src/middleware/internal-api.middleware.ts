...
export const internalApiAuth = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!apiKey) {
    logger.warn('Internal API call without API key', { path: req.path, method: req.method, ip: req.ip });
    ResponseUtil.unauthorized(res, 'Internal API key required');
    return;
  }

  if (apiKey !== expectedKey) {
    logger.warn('Internal API call with invalid API key', { path: req.path, method: req.method, ip: req.ip });
